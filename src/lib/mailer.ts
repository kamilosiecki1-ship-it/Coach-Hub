...
export async function sendInviteEmail(to: string, registrationLink: string, role: string): Promise<void> {
  ...
  const from = process.env.EMAIL_FROM ?? "Coach Hub <onboarding@resend.dev>";
  const roleLabel = role === "ADMIN" ? "administratora" : "coacha";

  await resend.emails.send({
    from,
    to,
    subject: "Zaproszenie do Coach Hub",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
        <h2 style="color:#1d4ed8">Zaproszenie do Coach Hub</h2>
        <p>Zostałeś zaproszony do platformy Coach Hub jako <strong>${roleLabel}</strong>.</p>
...
export async function sendPasswordResetEmail(to: string, resetLink: string): Promise<void> {
  ...
  await resend.emails.send({
    from,
    to,
    subject: "Coach Hub — reset hasła",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
        <h2 style="color:#1d4ed8">Reset hasła — Coach Hub</h2>
